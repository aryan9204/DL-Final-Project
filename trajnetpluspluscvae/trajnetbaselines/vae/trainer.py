...
from .vae import CVAE, VAEPredictor, drop_distant
...
class Trainer(object):
    def __init__(self, model=None, criterion=None, optimizer=None, lr_scheduler=None,
                 ...
                 augment_noise=False, alpha_kld=1.0, val_flag=True, kld_anneal_epochs=0):
        self.model = model if model is not None else CVAE()
        ...
        self.kld_anneal_epochs = kld_anneal_epochs

    def current_kld_weight(self, epoch):
        if self.kld_anneal_epochs <= 0:
            return self.alpha_kld
        progress = min(1.0, float(epoch + 1) / float(self.kld_anneal_epochs))
        return self.alpha_kld * progress
    ...
    def train(self, scenes, goals, epoch):
        ...
        for scene_i, (filename, scene_id, paths) in enumerate(scenes):
            ...
            if ((scene_i + 1) % self.batch_size == 0) or ((scene_i + 1) == len(scenes)):
                ...
                ## Train Batch
                losses = self.train_batch(batch_scene, batch_scene_goal, batch_split, epoch)
                epoch_loss += losses['loss']
                epoch_recon_loss += losses['recon_loss']
                epoch_kld_loss += losses['kld_loss']
            ...
            if (scene_i + 1) % (10*self.batch_size) == 0:
                self.log.info({
                    ...
                    'lr': self.get_lr(),
                    'loss': round(losses['loss'], 3),
                    'recon_loss': round(losses['recon_loss'], 3),
                    'kld_loss': round(losses['kld_loss'], 3),
                    'kld_weight': round(losses['kld_weight'], 3),
                })
        ...
        self.log.info({
            ...
            'recon_loss': round(epoch_recon_loss / len(scenes), 5),
            'kld_loss': round(epoch_kld_loss / len(scenes), 5),
            'kld_weight': round(self.current_kld_weight(epoch), 5),
    ...
    def val(self, scenes, goals, epoch):
        ...
        for scene_i, (filename, scene_id, paths) in enumerate(scenes):
            ...
            if ((scene_i + 1) % self.batch_size == 0) or ((scene_i + 1) == len(scenes)):
                ...
                losses = self.val_batch(batch_scene, batch_scene_goal, batch_split, epoch)
                val_loss += losses['teacher_forced_loss']
                test_loss += losses['prior_loss']
        ...
        self.log.info({
            ...
            'recon_loss': round(recon_loss / len(scenes), 3),
            'kld_loss': round(kld_loss / len(scenes), 3),
            'kld_weight': round(self.current_kld_weight(epoch), 5),
            'note': 'loss is teacher-forced CVAE objective; test_loss is prior-only reconstruction',
            'time': round(eval_time, 1),
        })

    def _reconstruction_loss(self, rel_outputs, targets, batch_split):
        recon_losses = []
        for rel_outputs_mode in rel_outputs:
            recon_losses.append(self.criterion(rel_outputs_mode[-self.pred_length:], targets, batch_split) * self.batch_size)
        return torch.stack(recon_losses).mean()

    def train_batch(self, batch_scene, batch_scene_goal, batch_split, epoch):
        ...
        rel_outputs, _, latent_params = self.model(observed, batch_scene_goal, batch_split, prediction_truth)

        reconstr_loss = self._reconstruction_loss(rel_outputs, targets, batch_split)
        kld_loss = self.kld_loss(
            latent_params['posterior_mu'],
            latent_params['posterior_log_var'],
            latent_params['prior_mu'],
            latent_params['prior_log_var'],
        ) * self.batch_size
        kld_weight = self.current_kld_weight(epoch)
        loss = reconstr_loss + kld_weight * kld_loss
        ...
        return {
            'loss': loss.item(),
            'recon_loss': reconstr_loss.item(),
            'kld_loss': kld_loss.item(),
            'kld_weight': kld_weight,
        }

    def val_batch(self, batch_scene, batch_scene_goal, batch_split, epoch):
        ...
        with torch.no_grad():
            ...
            kld_weight = self.current_kld_weight(epoch)
            teacher_forced_loss = reconstr_loss + kld_weight * kld_loss

            self.model.eval()
            rel_outputs_prior, _, _ = self.model(observed_test, batch_scene_goal, batch_split, n_predict=self.pred_length)
            prior_recon_loss = self._reconstruction_loss(rel_outputs_prior, targets, batch_split)
            self.model.train()

        return {
            'teacher_forced_loss': teacher_forced_loss.item(),
            'prior_loss': prior_recon_loss.item(),
            'recon_loss': reconstr_loss.item(),
            'kld_loss': kld_loss.item(),
        }
...
def main(epochs=25):
    ...
    hyperparameters.add_argument('--kld_anneal_epochs', type=int, default=0,
                                 help='linearly warm up KL weight over this many epochs; 0 disables annealing')
    ...
    hyperparameters.add_argument('--noise_dim', type=int, default=8,
                                 help='latent dimension of the CVAE')
    ...
    os.makedirs('OUTPUT_BLOCK/{}'.format(args.path), exist_ok=True)
    if args.goals:
    ...
    # Prefer CUDA when available, but preserve the existing opt-out flag.
    ...
    trainer = Trainer(model, optimizer=optimizer, lr_scheduler=lr_scheduler, device=args.device,
                      ...
                      val_flag=val_flag, kld_anneal_epochs=args.kld_anneal_epochs)
